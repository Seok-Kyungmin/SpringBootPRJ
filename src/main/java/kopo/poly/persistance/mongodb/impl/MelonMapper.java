...
public class MelonMapper extends AbstractMongoDBComon implements IMelonMapper {
	...
	public int insertSongMany(List<MelonDTO> pList, String colNm) throws Exception {
		...
		log.info(this.getClass().getName() + ".insertSongany Start!");
	...
	public int deleteSong(String pColNm, String pSinger) throws Exception{
		log.info(this.getClass().getName() + ".deleteSon Start!");
	...
	@Override
	public int updateSong(String pColNm, String pSinger, String pUdateSinger) throws Exception {
		...
		//조회할 조건(SQL의 WHERE 약활 / SELECT * FROM MELON_20220321 where singer ='방탄소년단')
		Document query = new Document();
		query.append("singer", pSinger);

		//MongoDB 데이터 삭제는 반드시 컬렉션을 조회하고, 조회된 ObjectID를 기반으로 데이터를 삭제함
		//MongoDB 환경은 분산환경(Sharding)ㅇ로 구성될 수 있기 때문에 정확한 PK에 매핑하기 위해서임
		FindIterable<Document> rs = col.find(query);
	...
	@Override
	public int updateSongAddFiled(String pColNm, String pSinger, String pNickname) throws Exception {

		log.info(this.getClass().getName() + ".updateSongAddFiled Start!");
		...
		log.info(this.getClass().getName() + ".updateSongAddFiled End!");

		return res;
	}

	@Override
	public int updateSongAddListFiled(String pColNm, String pSinger, List<String> pMember) throws Exception {
	...
	public int updateManySong(String pColNm, String pSinger, String pUpdateSinger, String pUpdateSong) throws Exception {
		...
		//조회할 조건(SQL의 WHERE 역할 / SELECT * FROM MELON_20220412 where singer ='방탄소년단')
		Document query = new Document();
		query.append("singer", pSinger);
		...
		// 한줄로 append에서 수정할 필드 추가해도 되지만, 가독성이 떨어져 줄마다 append 함
